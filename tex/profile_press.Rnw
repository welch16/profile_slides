...
\only<1>{
\begin{itemize}
\item The double stranded {\color{RoyalBlue} DNA}, we sonite and obtain fragments
\end{itemize}}
\only<2>{
\begin{itemize}
...
<<addCol2,include=TRUE,echo=TRUE,eval=FALSE,results="asis">>=
...
and the package is in {\color{RoyalBlue} \url{https://github.com/welch16/profile}}, or 
